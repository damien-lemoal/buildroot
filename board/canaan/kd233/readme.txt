...
the KD233 board can only be booted using the Direct Linux Kernel.

This is defined by the canaan_kd233_defconfig configuration. This configuration
allows building a bootable kernel image with a built-in initramfs root file
system (the board SD card is not used). The built kernel image can be flashed
directly to the board ROM for direct booting. No boot loader is required.

Once booted, the on-board SD card can be used by Linux.

The configuration file will also compile the kflash and pyserial-miniterm host
tools to respectively program bootable image files to the board and open a
